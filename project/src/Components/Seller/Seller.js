...
export default function Seller() {
  ...
  const sellerAddHandler = (newSeller) => {
    fetch("http://localhost:3001/seller", {
      ...
      .then(() => {
        alert(
          "Seller " + newSeller.firstName + " " + newSeller.surname + " added."
        );
      });
  };
  ...
  function handleDelete(sellerId, sellerFname, sellerLname) {
    const confirmMessage =
      "Are you sure you want to delete seller with \nREFERENCE: " +
      sellerId +
      "\nNAME: " +
      sellerFname +
      " " +
    ...
    if (confirmDelete) {
      fetch(`http://localhost:3001/seller/${sellerId}`, {
        ...
        .then((sellerToDelete) => {
          dispatch({ type: "REMOVE", payload: sellerToDelete });
        })
  ...
  return (
    <>
      <div className="pageHeader bg-dark">
      ...
      <ul className={"custom-list"}>
        {sellersList.map((seller) => (
          <li key={seller.id} data-cy="sellerDetails">
            <div className="sellerBlock bg-dark-subtle">
              {" "}
            ...
            <div>
              ...
              <div className={"float-end text-white"}>
                ...
                <Link
                  to={`${seller.id}/property`}
                  state={seller}
                  className="btn btn-warning btn-sm"
                  data-cy="managePropertyBtn"
                >
                ...
                <button
                  ...
                  onClick={() =>
                    handleDelete(seller.id, seller.firstName, seller.surname)
                  }
                >
      ...
      {show && (
        <Modal show={show} onHide={handleClose}>
          ...
          <Modal.Body>
            <form className={"bg-light text-dark"}>
              ...
              <input
                ...
                id={"fname"}
                defaultValue={sellerToEdit.firstName}
                ref={firstNameRef}
                data-cy="profilefname"
              />
              <label>Surname</label>
              <input
                type="text"
                className="form-control"
                id={"sname"}
                defaultValue={sellerToEdit.surname}
                ref={surnameRef}
                data-cy="profilelname"
              />
              <label>Address</label>
              <input
                type="text"
                className="form-control"
                id={"address"}
                defaultValue={sellerToEdit.address}
                ref={addressRef}
              ...
              <input
                ...
                className="form-control"
                id={"postcode"}
                defaultValue={sellerToEdit.postcode}
                ref={postcodeRef}
                data-cy="profilepostcode"
              />
              <label>Phone</label>
              <input
                type="text"
                className="form-control"
                id={"phone"}
                defaultValue={sellerToEdit.phone}
                ref={phoneRef}
          ...
          <Modal.Footer>
            ...
            <Button
              variant="primary"
              onClick={() => handleUpdate(sellerToEdit.id)}
              data-cy="saveChangesBtn"
            >
              Save Changes
            </Button>
          </Modal.Footer>
